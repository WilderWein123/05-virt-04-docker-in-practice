...
Не понял, как скормить докеру файл, который не Dockerfile. Соответтсвенно либо создавать директорию, в которой уже будет лежать докерфайл, либо сохранять его не как Dockerfile.pyhon, а как Dockerfile. Решил пойти по второму пути. 
Сборка завершилась успешно, о чем намекает последняя строка:
...
Приложение не хотело заводиться без модуля dotenv, неистово сыпля в лог ошибками. Рекомендовало выполнить его установку через pip install. Но кажется в файл requrements.txt добавить его правильнее, в связи с чем я это сделал.
...
docker compose up -d
```
Форк я не делал, то что должно быть форк-репозиторием я склонировал в свой - https://github.com/WilderWein123/05-virt-04-docker-in-practice/tree/main/shvirtd-example-python
Скриншот выполнения и проверки:
<img src = "img/task4.png" width = 100%>
<img src = "img/task4-2.png" width = 100%>
